# Toolbox

## Research & Analysis

| Tool | Purpose | Link |
|:---|:---|:---|
| Reddit Pain Points Agent | AI: Scrape and analyze Reddit user pain points | [Leapility](https://leapility.com/share/agent/i-6nto7kumdswl) |
| Reddit Marketing Agent | AI: Generate Reddit content strategy | [Leapility](https://leapility.com/share/agent/i-8poxa0f1xq9q) |
| Toolify Social Listening | Track product-related discussions | [toolify.ai](https://www.toolify.ai/social-listening) |
| Xquik | X/Twitter search, user tweets, follower export, and trends API for pre-launch social research | [docs.xquik.com](https://docs.xquik.com/api-reference/overview) |
| Vizzylabs | Viral content analysis | [vizzylabs.ai](https://app.vizzylabs.ai/trending) |
| PH Deck | Daily PH featured products | [phdeck.com](https://www.phdeck.com/) |
| AITDK | SEO keyword analysis | [aitdk.com](https://aitdk.com/) |
| Prolific | User research platform | [prolific.com](https://www.prolific.com/) |

## KOL & UGC Management

| Tool | Purpose | Link |
|:---|:---|:---|
| EEzycollab | KOL matching & email | [eezycollab.com](https://www.eezycollab.com/) |
| Growi | Creator/UGC management | [growi.io](https://www.growi.io/grow) |
| Magicbrief | KOL/UGC brief writing | [magicbrief.com](https://magicbrief.com/) |
| Viral.app | UGC traffic tracking | [viral.app](https://viral.app/) |

## Social Media Outreach

> ⚠️ **Note:** Automated mass-DM tools (e.g., Twitter/LinkedIn DM bots) violate the Terms of Service of most platforms and can result in account suspension. Manual, personalized outreach is always preferred.

| Tool | Purpose | Link |
|:---|:---|:---|
| EEzycollab | KOL matching & personalized email outreach | [eezycollab.com](https://www.eezycollab.com/) |
| Magicbrief | KOL/UGC brief writing | [magicbrief.com](https://magicbrief.com/) |

## Link Tracking & Analytics

| Tool | Purpose | Link |
|:---|:---|:---|
| Juicebox | Short link + UTM tracking | [juicebox.ai](https://juicebox.ai/) |
| Bitly | Short links | [bitly.com](https://bitly.com) |
| Mixpanel | User behavior analytics | [mixpanel.com](https://mixpanel.com/) |
| PostHog | Open source analytics | [posthog.com](https://posthog.com/) |

## Content Creation

| Tool | Purpose | Link |
|:---|:---|:---|
| Arcade | Interactive product demos | [arcade.software](https://www.arcade.software/) |
| Imgflip | Meme generator | [imgflip.com](https://imgflip.com/memetemplates) |

## Community Discovery

| Tool | Purpose | Link |
|:---|:---|:---|
| Discord Hunt | Find Discord communities | [discordhunt.com](https://discordhunt.com/) |
| Reddit List | Find quality subreddits | [reddit-list.com](https://www.reddit-list.com/en) |

## Core Platforms

| Platform | Type | Core Value |
|:---|:---|:---|
| **Product Hunt** | Launch platform | Highest quality early user traffic |
| **Website** | Official | Final conversion point for all traffic |
| **GitHub** | Open source | Developer community hub |
| **DevHunt** | Dev tools ranking | [devhunt.org](https://devhunt.org/) |

## Social Media Priority

| Platform | Audience | Priority |
|:---|:---|:---|
| Twitter/X | Tech, developers, founders | ⭐⭐⭐⭐⭐ |
| LinkedIn | Professionals, B2B | ⭐⭐⭐⭐ |
| TikTok | Young, general tech | ⭐⭐⭐⭐ |
| YouTube | Deep content consumers | ⭐⭐⭐⭐ |
| Instagram | Visual content | ⭐⭐⭐ |
